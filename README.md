...
1. **Contactos reales (BBDD)** — `src/data/contactosReales.ts`: **612 contactos** combinados desde dos fuentes (campo `origen`): la base de LinkedIn de ARCANCHILE (`BBDD_LINKEDIN_2025-2026.xlsx`, 485) y la **nómina de asistentes al evento Pipeline II de MineGroup** (mayo 2026, 127 — leads tibios del ecosistema de ductos, todos con correo). Se filtró a empresas del sector (minería, EPC/ingeniería, energía, ductos, agua, celulosa, oil&gas, inspección, industrial) y se dedujeron duplicados por correo o nombre+empresa. Cada contacto tiene nombre, teléfono (enlaces a llamada y WhatsApp), correo (enlace mailto), empresa, cargo, `nivel`/`prioridad` derivados del cargo, `sector`, `origen`, `nota` y un **estado CRM inicial derivado de la gestión original** (p. ej. "AGENDADO" → Reunión agendada, "PRESENTACIÓN ENVIADA" → Propuesta, "ARCAN" → En seguimiento; los del evento entran como Pendiente). 324 contactos se vinculan automáticamente a los proyectos cuya empresa coincide.
